...
async def get_profile(
    ...
    draft = (
        await db.execute(
            select(ProfileDraft)
            .where(ProfileDraft.customer_id == cid, ProfileDraft.status == "draft")
...
async def confirm_profile(
    ...
    if body.mode == "all" or all(
        (draft.field_status or {}).get(f) == "confirmed"
        for f in ("basic_info", "study_info", "prefer_info", "timeline")
    ):
        draft.status = "merged"
    ...
    await write_event(
        ...
        ref_type="profile_draft",
        ref_id=draft.id,
        meta={"fields": fields, "mode": body.mode},
    )
    await db.commit()
    return ok({"profile_version": profile.version, "draft_status": draft.status})
